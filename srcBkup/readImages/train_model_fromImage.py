...
x = Conv2D(32, (3, 3))(image_input)  # Convolutional layer with 32 filters
x = BatchNormalization()(x)          # Batch normalization
#x = Activation('relu')(x)            # Activation layer
x = LeakyReLU(alpha=0.01)(x)         # Leaky ReLU with a small negative slope (alpha=0.01)
x = MaxPooling2D((2, 2))(x)          # Max-pooling layer

# Second Convolutional Block
x = Conv2D(64, (3, 3))(x)            # Convolutional layer with 64 filters
x = BatchNormalization()(x)          # Batch normalization
#x = Activation('relu')(x)            # Activation layer
x = LeakyReLU(alpha=0.01)(x)         # Leaky ReLU with a small negative slope (alpha=0.01)
x = MaxPooling2D((2, 2))(x)          # Max-pooling layer
...
x = Dense(128)(x)                    # Fully connected layer with 128 units
x = BatchNormalization()(x)          # Batch normalization
#x = Activation('relu')(x)            # Activation layer
x = LeakyReLU(alpha=0.01)(x)         # Leaky ReLU with a small negative slope (alpha=0.01)
...
# Define EarlyStopping to monitor validation accuracy with a patience of 5 epochs
...
history = model.fit(
    ...
    callbacks=[checkpoint, early_stopping]  # Add EarlyStopping to the callbacks
)
